    ...
    static func generateInsights(
        ...
        if weekCompleted {
            insights.append(SmartInsight(
                ...
                message: String(localized: "You've completed \(workoutsThisWeek) workouts this week. Consistency like this is what drives real results. Enjoy the rest of the week or keep pushing!"),
                iconAsset: "trophy",
                priority: 88
            ))
        }
        ...
        if let daysSince = daysSinceLastWorkout, daysSince >= 4 {
            insights.append(SmartInsight(
                ...
                message: String(localized: "It's been \(daysSince) days since your last session. Getting back in the gym helps maintain the progress you've built. Even a light workout counts."),
                iconAsset: "flame",
                priority: 95
            ))
        ...
        if !weekCompleted && workoutsThisWeek == workoutsRequired - 1 && workoutsRequired > 1 {
            insights.append(SmartInsight(
                ...
                message: currentStreak > 0
                    ? String(localized: "Just 1 more workout to keep your \(currentStreak)-week streak alive. You've got this!")
                    : String(localized: "Just 1 more workout this week to start building a streak."),
                iconAsset: "flame",
                priority: 83
            ))
        }
        ...
        if dayOfMonth >= 8 && lastMonthWorkouts > 0 && monthlyWorkouts > lastMonthWorkouts {
            ...
            if increase >= 20 {
                insights.append(SmartInsight(
                    ...
                    message: String(localized: "You've already completed \(monthlyWorkouts) workouts so far this month, compared to \(lastMonthWorkouts) all of last month. Consistency is the strongest predictor of long-term strength gains."),
        ...
        if dayOfMonth >= 8 && lastMonthVolume > 0 && monthlyVolume > lastMonthVolume {
            ...
            if increase >= 10 {
                ...
                insights.append(SmartInsight(
                    ...
                    message: String(localized: "Your training volume so far this month is already up \(formattedIncrease) compared to all of last month. Gradual progressive overload is the primary driver of muscle growth."),
                    iconAsset: "scale",
                    priority: 70
                ))
            }
        ...
        if dayOfMonth >= 8 && lastMonthVolume > 0 && monthlyVolume < lastMonthVolume {
            ...
            if decrease >= 20 {
                ...
                insights.append(SmartInsight(
                    ...
                    message: String(localized: "Your training volume so far this month is down \(formattedDecrease) compared to last month. If you're deloading intentionally that's great, otherwise try to maintain or gradually increase load."),
                    iconAsset: "scale",
                    priority: 58
                ))
            }
        ...
        if avgDuration > 90 && monthlyWorkouts >= 2 {
            insights.append(SmartInsight(
                ...
                message: String(localized: "Your average session is \(avgDuration) minutes. Longer sessions can be effective, but if you're feeling drained, consider higher-intensity shorter sessions to stay fresh."),
                iconAsset: "stopwatch",
                priority: 55
            ))
        }
        ...
        if currentStreak == 0 && previousWeekStreak >= 3 {
            insights.append(SmartInsight(
                ...
                message: String(localized: "Your \(previousWeekStreak)-week streak ended, but the strength you built didn't disappear. Get \(workoutsRequired) workouts in this week to start a new one."),
                iconAsset: "flame",
                priority: 82
            ))
        }
